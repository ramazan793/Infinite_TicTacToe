import java.util.*; 
final color CYAN = color(0,191,255); 
int Scale =50; 
ArrayList<Toe> jesus; 
int count =2; 
int acc2=0;
...
class Tic extends Toe { 
   int status =1; 
   float d = Scale/6.25; 
   color c = color(255,0,0,300); 
   int w = 3; 
   Tic(int mx, int my,color c, int w){ 
      this.x=(mx/25)*25; 
      this.y=(my/25)*25; 
      this.c=c; 
      this.w=w; 
...
class Tac extends Toe { 
   ...
   color c = color(255,0,0,300); 
   int w = 3; 
   Tac(int mx, int my,color c, int w){ 
      this.x=(mx/25)*25; 
      this.y=(my/25)*25; 
      this.c=c; 
      this.w=w; 
   ...
   void display(){ 
      ...
      stroke(1,1,1); 
      strokeWeight(w); 
      ellipse((float)this.x+(float)Scale/2, (float)this.y+(float)Scale/2, Scale/1.56, Scale/1.56); 
      strokeWeight(1); 
      stroke(CYAN); 
...
boolean ToeChecker(int x, int y, int status){ 
   for (int i=0;i<jesus.size();i++){ 
...
boolean Rogue(int x,int y, int status,int acc,int dy,int dx){ 
